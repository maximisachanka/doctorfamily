    ...
    try {
        ...
        const results = [
            ...services.map((service)=>{
                ...
                const serviceSlug = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$utils$2f$serviceMapping$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getServiceSlugByTitle"])(service.category.slug, service.title);
                // Если slug найден, используем полный URL, иначе перенаправляем на страницу услуг
                const url = serviceSlug ? `/services/${service.category.slug}/${serviceSlug}` : `/services`;
                return {
        ...
        const patientPageResults = patientPageKeywords.filter((item)=>item.keywords.some((keyword)=>keyword.includes(searchTerm.toLowerCase()) || searchTerm.toLowerCase().includes(keyword))).map((item, index)=>({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__99be39a8._.js.map