...
const tutorialSlides = [
  {
    id: 1,
    title: "Добро пожаловать в раздел услуг!",
    content: "Здесь вы можете найти всю информацию о наших медицинских услугах. Сейчас покажем, как быстро найти то, что вам нужно.",
    illustration: (
      <div className="w-full h-48 bg-[#18A36C] rounded-lg flex items-center justify-center relative overflow-hidden">
        <div className="text-center text-white">
          ...
          <h3 className="text-lg font-medium">Медицинские услуги</h3>
          <p className="text-sm text-white/80 mt-1">Полный спектр медицинской помощи</p>
        </div>
      </div>
    )
  },
  {
    id: 2,
    title: "Откройте меню услуг",
    content: "Нажмите на круглую кнопку меню в левом верхнем углу, чтобы увидеть все доступные категории медицинских услуг.",
    illustration: (
  ...
  },
  {
    ...
    content: "В меню все услуги разделены по направлениям. Выберите интересующую вас категорию для получения подробной информации.",
    illustration: (
      <div className="w-full h-48 bg-white rounded-lg border border-[#CACACA] overflow-hidden relative">
        <div className="bg-[#18A36C] p-3">
          <h3 className="text-white text-sm font-medium">Медицинские услуги</h3>
          <p className="text-white/80 text-xs">Выберите нужное направление</p>
        </div>
        
        <div className="p-2 space-y-1">
          {[
            { name: 'Стоматология', icon: '🦷', highlight: true },
            { name: 'Гинекология', icon: '👩‍⚕️', highlight: false },
            { name: 'УЗИ диагностика', icon: '📱', highlight: false },
            { name: 'Эндокринология', icon: '🔬', highlight: false }
          ].map((service, index) => (
            <div 
              key={service.name} 
