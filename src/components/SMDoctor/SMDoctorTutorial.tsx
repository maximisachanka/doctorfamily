...
import { ChevronLeft, ChevronRight, Menu, UserCheck, X } from "lucide-react";

interface DoctorsTutorialProps {
  isOpen: boolean;
  onClose: () => void;
}
...
const tutorialSlides = [
  {
    id: 1,
    title: "Добро пожаловать к нашим специалистам!",
    content: "Здесь вы найдете информацию о всех врачах клиники, их специализациях и квалификациях. Покажем, как найти нужного специалиста.",
    illustration: (
      <div className="w-full h-48 bg-[#18A36C] rounded-lg flex items-center justify-center relative overflow-hidden">
        <div className="text-center text-white">
          ...
          <h3 className="text-lg font-medium">Наши специалисты</h3>
          <p className="text-sm text-white/80 mt-1">Опытные врачи для вашего здоровья</p>
        </div>
      </div>
    )
  },
  {
    id: 2,
    title: "Откройте меню специалистов",
    content: "Нажмите на кнопку меню, чтобы увидеть все врачебные специализации и найти нужного доктора.",
    illustration: (
      <div className="w-full h-48 bg-gray-100 rounded-lg relative border-2 border-[#CACACA] overflow-hidden">
        {/* Mock mobile screen with header */}
        ...
        {/* FAB button with arrow - убираем анимацию bounce */}
  ...
  },
  {
    ...
    content: "Найдите нужную специализацию и выберите врача. У каждого доктора есть подробная информация о квалификации и опыте работы.",
    illustration: (
      <div className="w-full h-48 bg-white rounded-lg border border-[#CACACA] overflow-hidden relative">
        {/* Menu header */}
        <div className="bg-[#18A36C] p-3">
          <h3 className="text-white text-sm font-medium">Специалисты</h3>
          <p className="text-white/80 text-xs">Выберите врача по специализации</p>
        </div>
        
        {/* Menu items */}
        <div className="p-2 space-y-1">
          {[
            { name: 'Стоматологи', icon: '🦷', highlight: true },
            { name: 'Гинекологи', icon: '👩‍⚕️', highlight: false },
            { name: 'Специалисты УЗИ', icon: '📱', highlight: false },
            { name: 'Эндокринологи', icon: '🔬', highlight: false }
          ].map((service, index) => (
            <div 
              key={service.name} 
...
export function DoctorsTutorial({ isOpen, onClose }: DoctorsTutorialProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md mx-auto p-0 overflow-hidden border-2 border-[#18A36C]/20 shadow-2xl">
        <DialogTitle className="sr-only">Обучение по разделу Специалисты</DialogTitle>
        <DialogDescription className="sr-only">
          Руководство по навигации в разделе врачей и специалистов Doctor Family
        </DialogDescription>
        <div className="relative bg-gradient-to-br from-white to-[#F4F4F4]/50">
          {/* Close button - исправляем вертикальное центрирование */}
          <Button
            variant="ghost"
            size="sm"
            onClick={onClose}
